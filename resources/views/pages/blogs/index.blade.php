@extends('layouts.web')
@section('title', 'Blogs - Dissertation Writing Help')
@section('description', '')
...
<section class="px-4 md:px-16 lg:px-24 py-14 ">
    <div class="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-2 gap-y-4">
        ...
        <div class="">
            <a href="{{ route('blogs.show', $blog->slug) }}"><img src="{{ asset('storage/'.$blog->image_path) }}"  alt="{{ env('APP_NAME') }}" class="h-72 w-full border-8 border-gray-100 hover:scale-105 hover:opacity-70 transition ease-in-out delay-150 cursor-pointer"></a>
            <p class="bg-white py-4 mx-auto md:mx-0"> <i class="fa fa-user px-1"></i> admin on <i class="fa fa-clock-o px-1"></i> July 28, 2021</p>
            <a href="{{ route('blogs.show', $blog->slug) }}"><p class="bg-white text-2xl w-full text-center md:text-left mt-4 sm:mt-0 h-14">{{ $blog->title }}</p></a>
